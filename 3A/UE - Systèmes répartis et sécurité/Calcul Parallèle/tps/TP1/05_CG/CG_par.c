...
void cg_par(double *A_local, double *rhs_local, int N, int b, float tol){
  ...
  nr_local = dot(rhs_local, rhs_local, b); //** 1
  MPI_Allreduce(&nr_local, &nr_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); //** 2
  nr_global = sqrt(nr_global); //** 3
  ...
  while((nr_global > epsilon) && (num_it< max_it)){
    ...
    MPI_Allgather(p_local, b, MPI_DOUBLE, p_global, b, MPI_DOUBLE, MPI_COMM_WORLD); //** 4

    // do the matrix-vector multiplication
    multAv(Ap, A_local, p_global, b, N); //** 5

    // compute the global dot product np2_global = (Ap_global, p_global)
    // all the node must have this value
    np2_local = dot(p_local, Ap, b); //** 6
    MPI_Allreduce(&np2_local, &np2_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); //** 7
    ...
    // all the nodes must have this value
    nr_local = dot(r, r, b); //** 8
    MPI_Allreduce(&nr_local, &nr_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); //** 9
    nr_global = sqrt(nr_global); //** 10
    //if(my_rank == 0) printf("nr = %lg\n", nr_global);
  ...
  double x_global[N];
  MPI_Gather(x, b, MPI_DOUBLE, x_global, b, MPI_DOUBLE, 0, MPI_COMM_WORLD); //** 10
